jQuery(function ($) {
	function wpgmza_InitMap() {
		var myLatLng = new google.maps.LatLng(
				wpgmza_legacy_polyline_panel_vars.wpgmza_lat,
				wpgmza_legacy_polyline_panel_vars.wpgmza_lng);

		MYMAP.init('#wpgmza_map', myLatLng, wpgmza_legacy_polyline_panel_vars.start_zoom);
	}

	jQuery("#wpgmza_map").css({
		height: wpgmza_legacy_polyline_panel_vars.wpgmza_height + wpgmza_legacy_polyline_panel_vars.wpgmza_height_type,
		width: wpgmza_legacy_polyline_panel_vars.wpgmza_width + wpgmza_legacy_polyline_panel_vars.wpgmza_width_type
	});
...
MYMAP.init = function (selector, latLng, zoom)
{
	var myOptions = {
		...
		mapTypeId: google.maps.MapTypeId[wpgmza_legacy_polyline_panel_vars.wpgmza_map_type]
	};
	var data, existingPolyline, marker, markers = [];
	
	this.map = new google.maps.Map(jQuery(selector)[0], myOptions);
	this.bounds = new google.maps.LatLngBounds();
	
	var targetPolylineID = WPGMZA.getQueryParamValue("poly_id");
	
	for(var poly_id in wpgmza_legacy_polyline_panel_vars.polyline_data)
	{
		data = wpgmza_legacy_polyline_panel_vars.polyline_data[poly_id];
		data.path = new google.maps.MVCArray(WPGMZA.LatLng.toGoogleLatLngArray(data.path));
		
		existingPolyline = new google.maps.Polyline(data);
		existingPolyline.setMap(this.map);
		
		if(poly_id == targetPolylineID)
		{
			poly_path = data.path;
			poly = existingPolyline;
		}
	}
	...
	{
		data = wpgmza_legacy_polyline_panel_vars.polyline_data[targetPolylineID];
